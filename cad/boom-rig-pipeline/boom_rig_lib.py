"""
Shared extraction core, factored out of extract_t3f.py so the same
per-axis-pair logic (`extract_grid()`, pivot/ground detection) can be reused
across every LTM 1650 T3-family config instead of copy-pasting per config.
See extract_t3f.py / extract_all_poses.py / the README for the full pipeline
context - this module is just the DXF-in, grid-JSON-out mechanics.
"""
...
def extract_grid(label, lengths, angles, fname_pattern):
